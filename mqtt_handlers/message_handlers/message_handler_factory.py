...
from mqtt_handlers.message_handlers.home_assistant_handler import (
    HomeAssistantMessageHandler,
)
...
class MessageHandlerFactory:
    ...
    def __create_message_handler(
        self, preferred_device_type: DeviceType | None = None
    ) -> MessageHandlerPort:
        device_type = (
            preferred_device_type
            if preferred_device_type is not None
            else self.subscriberData.deviceType
        )
        if device_type == DeviceType.GENERIC_RF_433:
            return GenericRtlMessageHandler(
                self.subscriberData, self.appSettings, self.publisher, self.logger
            )
        elif device_type == DeviceType.ENERGY_METER:
            return EnergyMessageHandler(
                self.subscriberData, self.appSettings, self.publisher, self.logger
            )
        elif device_type == DeviceType.HOME_ASSISTANT:
            return HomeAssistantMessageHandler(
                self.subscriberData, self.appSettings, self.publisher, self.logger
            )
        elif device_type == DeviceType.LEAK_SENSOR:
            return LeakSensorHandler(
                self.subscriberData, self.appSettings, self.publisher, self.logger
            )
        else:
            raise ValueError(
                f"Unsupported sensor type: {self.subscriberData.deviceType}"
            )
    ...
    def close(self):
        ...
        self.logger.info(
            "Closed subscriber for device type: %s", self.subscriberData.deviceType
        )
